...
export function EditLoanModal({ 
  ...
  const { updateLoan, isLoading, error } = useLoans()
  ...
  const [formData, setFormData] = useState({
    ...
    status: 'PENDING' as 'PENDING' | 'APPROVED' | 'REJECTED' | 'ACTIVE' | 'COMPLETED' | 'DEFAULTED'
  })
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const updateData = {
      ...
      baseInterestRate: formData.baseInterestRate ? parseFloat(formData.baseInterestRate) / 100 : undefined, // Convertir a decimal
      penaltyInterestRate: formData.penaltyInterestRate ? parseFloat(formData.penaltyInterestRate) / 100 : undefined,
    ...
    console.log('Status actual:', loan.status, '→ Nuevo status:', formData.status)
