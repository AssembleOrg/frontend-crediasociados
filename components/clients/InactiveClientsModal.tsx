...
export default function InactiveClientsModal({ open, onClose }: InactiveClientsModalProps) {
  ...
  useEffect(() => {
    if (open && selectedManager) {
      loadInactiveClients()
    } else if (open && !selectedManager) {
      setClients([])
      setTotalClients(0)
      setError(null)
    }
  ...
  const loadInactiveClients = async () => {
    ...
    setError(null)
    try {
      const data = await clientsService.getInactiveClients(selectedManager)
  ...
  return (
    ...
    >
      ...
      <DialogContent sx={{ p: { xs: 2, sm: 3 }, bgcolor: 'background.default' }}>
        ...
        {/* Search Input */}
        {selectedManager && !loading && !error && clients.length > 0 && (
          <Box sx={{ mb: 3 }}>
            <TextField
              fullWidth
        ...
        {/* Empty State - No Manager Selected */}
